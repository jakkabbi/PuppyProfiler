...
describe('Component: App', () => {
  ...
  it('should display the navigation bar correctly for guests', () => {
    ...
    expect(de[0].nativeElement.textContent).toContain('Login');
    // expect(de[1].nativeElement.textContent).toContain('Domains');
    expect(de[1].nativeElement.textContent).toContain('Register');
    expect(de[0].attributes['routerLink']).toBe('/login');
    // expect(de[1].attributes['routerLink']).toBe('/domains');
    expect(de[1].attributes['routerLink']).toBe('/register');
  });
